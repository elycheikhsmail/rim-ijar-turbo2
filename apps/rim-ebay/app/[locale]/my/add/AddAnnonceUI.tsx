...
export default function AddAnnonceUI({ lang = "ar", userid }: { lang?: string; userid: string}) {
  ...
  useEffect(() => {
    const fetchCategories = async () => { 
      if (selectedTypeId !== undefined) {
        try {
          const response = await axios.get(`/${lang}/api/categories?typeAnnonceId=${encodeURIComponent(selectedTypeId)}`);
  ...
  useEffect(() => {
    const fetchSubCategories = async () => {
      if (selectedCategoryId !== undefined) {
        try {
          const response = await axios.get(`/${lang}/api/subCategories?CategoryId=${encodeURIComponent(selectedCategoryId)}`);
  ...
  const handleTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const selectedType = typeAnnonces.find(type => type.id === String(typeId));
    if (selectedType) {
  ...
  const handleSubCategoryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const subCategory = filteredSubCategories.find(subCat => String(subCat.id) === subCategoryId);
    if (subCategory) {
