...
const handleGetAnnonces: (filters?: Filters) => Promise<{ pageAnnonceData: IPageAnnonce | null; errorMessage: string }> = 
    ...
    try {
        ...
        const annonces: Annonce[] = annoncesFromDB.map(annonce => {
            const { categorie, typeAnnonce, subcategorie, firstImagePath, ...rest } = annonce;
